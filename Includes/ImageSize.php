...
//		var_dump( $image_attributes);
	...
	public static function get_elementor_attachment( array $args ) {
		...
		if ( empty( $settings ) ) {
			_doing_it_wrong( sprintf( '%s::%s', get_called_class(), __FUNCTION__ ), esc_html__( 'Cannot get attachment because missing elementor widget settings.', 'gmart-core' ), '1.0.0' );

			return false;
		}

		if ( empty( $settings["{$image_key}"] ) ) {
			_doing_it_wrong( sprintf( '%s::%s', get_called_class(), __FUNCTION__ ), sprintf( esc_html__( 'Cannot get attachment because image key: %s is not exits.', 'gmart-core' ), $image_key ), '1.0.0' );
		...
		if ( empty( $image['url'] ) && empty( $image['id'] ) ) {
			_doing_it_wrong( sprintf( '%s::%s', get_called_class(), __FUNCTION__ ), sprintf( esc_html__( 'Cannot get attachment because image key: %s has no both id & url.', 'gmart-core' ), $image_key ), '1.0.0' );
