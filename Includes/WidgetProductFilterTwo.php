 ...
 */
class WidgetProductFilterTwo extends \WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'gmart-products-filter',
			esc_html__( 'Razzi - Products Filter', 'gmart' ),
			array(
				'classname'                   => 'products-filter-widget woocommerce',
				'description'                 => esc_html__( 'WooCommerce products filter.', 'gmart' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['filter'] ) ) {
			...
			echo '<button type="submit" value="' . esc_attr__( 'Filter', 'gmart' ) . '" class="button filter-button">' . esc_html__( 'Filter', 'gmart' ) . '</button>';
			echo '<button type="reset" value="' . esc_attr__( 'Reset Filter', 'gmart' ) . '" class="button alt reset-button button-lg">' . esc_html__( 'Reset Filter', 'gmart' ) . '</button>';
			echo '</div>';
		...
		foreach ( $active_filters as $filter ) {
			...
			foreach ( $terms as $term ) {
				switch ( $filter['source'] ) {
					...
					case 'rating':
						$text = _n( 'Rated %d star', 'Rated %d stars', $term, 'gmart' );
	...
	public function display_filter( $filter ) {
		...
		if ( 'attribute' == $filter['source'] ) {
			...
			$args['all']        = sprintf( esc_html__( 'Any %s', 'gmart' ), wc_attribute_label( $attr->attribute_label ) );
			...
			$args['all'] = sprintf( esc_html__( 'Select a %s', 'gmart' ), $taxonomy->labels->singular_name );
		} else {
			$args['all'] = esc_html__( 'All Products', 'gmart' );
		}
		...
		<div class="<?php echo esc_attr( join( ' ', $classes ) ) ?>">
			...
			<div class="products-filter__filter-control filter-control">
				...
				<div class="products-filter__control-buttons">
					<button type="button" value="<?php esc_attr_e( 'Clear', 'gmart' ); ?>"
							class="button alt clear-button"><?php esc_html_e( 'Clear', 'gmart' ); ?></button>
					<button type="submit" value="<?php esc_attr_e( 'Show Results', 'gmart' ) ?>"
							class="button filter-button button-lg"><?php esc_html_e( 'Show Results', 'gmart' ); ?></button>
				</div>
			</div>
		</div>
	...
	protected function get_filter_options( $filter ) {
		...
		switch ( $filter['source'] ) {
			...
			case 'products_group':
				$filter_groups = apply_filters( 'dmt_products_filter_groups', array(
					'best_sellers' => esc_html__( 'Best Sellers', 'gmart' ),
					'new'          => esc_html__( 'New Products', 'gmart' ),
					'sale'         => esc_html__( 'Sale Products', 'gmart' ),
					'featured'     => esc_html__( 'Hot Products', 'gmart' ),
				) );

				if ( 'dropdown' != $filter['display'] ) {
					$options[''] = array(
						'name'  => esc_html__( 'All Products', 'gmart' ),
						'count' => 0,
						'id'    => 0,
						'level' => 0,
					);
			...
			case 'orderby':
				$orderby = apply_filters( 'dmt_products_filter_orderby', array(
					'menu_order' => esc_html__( 'Default sorting', 'gmart' ),
					'popularity' => esc_html__( 'Sort by popularity', 'gmart' ),
					'rating'     => esc_html__( 'Sort by average rating', 'gmart' ),
					'date'       => esc_html__( 'Sort by latest', 'gmart' ),
					'price'      => esc_html__( 'Sort by price: low to high', 'gmart' ),
					'price-desc' => esc_html__( 'Sort by price: high to low', 'gmart' ),
				) );
			...
			case 'product_status':
				...
				if(! isset($filter['disable_in_stock'])) {
					$options[ 'in_stock' ] = array(
						'name'  => esc_html__( 'In Stock', 'gmart' ),
						'count' => 0,
						'id'    => 'in_stock',
						'level' => 0,
					);
	...
	protected function filter_search_box( $filter ) {
		if ( 'attribute' == $filter['source'] ) {
			...
			$placeholder = __( 'Search', 'gmart' ) . ' ' . strtolower( $attributes[ $filter['attribute'] ] );
		} else {
			$sources     = $this->get_filter_source_options();
			$placeholder = __( 'Search', 'gmart' ) . ' ' . strtolower( $sources[ $filter['source'] ] );
		}
	...
	public function form( $instance ) {
		...
		$this->setting_field( array(
			'type'  => 'text',
			'name'  => 'title',
			'label' => esc_html__( 'Title', 'gmart' ),
			'value' => $instance['title'],
		) );
		?>

		<div class="gmart-products-filter-form__sub-nav">
			<button type="button" data-section="filters"
					class="button-link active"><?php esc_html_e( 'Filters', 'gmart' ); ?></button>
			|
			<button type="button" data-section="options"
					class="button-link"><?php esc_html_e( 'Options', 'gmart' ); ?></button>
		</div>

		<p>
		<hr/></p>

		<div class="gmart-products-filter-form__section active" data-section="filters">
			<p class="gmart-products-filter-form__message <?php echo ! empty( $instance['filter'] ) ? 'hidden' : '' ?>"><?php esc_html_e( 'There is no filter yet.', 'gmart' ) ?></p>

			<div class="gmart-products-filter-form__filter-fields">
				<?php $this->filter_setting_fields( $instance['filter'] ); ?>
			</div>

			<p class="gmart-products-filter-form__section-actions">
				<button type="button" class="gmart-products-filter-form__add-new button-link"
						data-name="<?php echo esc_attr( $this->get_field_name( 'filter' ) ); ?>"
						data-count="<?php echo count( $instance['filter'] ) ?>">
					+ <?php esc_html_e( 'Add a new filter', 'gmart' ) ?></button>
			</p>
		</div>

		<div class="gmart-products-filter-form__section" data-section="design">
		</div>

		<div class="gmart-products-filter-form__section" data-section="options">
			<?php
			$this->setting_field( array(
				'type'  => 'checkbox',
				'name'  => 'ajax',
				'label' => esc_html__( 'Use ajax for filtering', 'gmart' ),
				'value' => $instance['ajax'],
			) );
			...
			$this->setting_field( array(
				'type'      => 'checkbox',
				'name'      => 'instant',
				'label'     => esc_html__( 'Filtering products instantly (no buttons required)', 'gmart' ),
			...
			$this->setting_field( array(
				'type'      => 'select',
				'name'      => 'filter_buttons',
				'label'     => esc_html__( 'Filter buttons position', 'gmart' ),
				'value'     => $instance['filter_buttons'],
				'options'   => array(
					'form'   => __( 'Bottom of form', 'gmart' ),
					'fitems' => __( 'Bottom of filter items', 'gmart' ),
				),
				'condition' => array(
					'instant' => false,
				),
			...
			$this->setting_field( array(
				'type'      => 'checkbox',
				'name'      => 'change_url',
				'label'     => esc_html__( 'Update URL', 'gmart' ),
	...
	protected function filter_setting_fields( $fields = array(), $context = 'display' ) {
		...
		foreach ( $filter_fields as $index => $field ) :
			...
			?>
			<div class="gmart-products-filter-form__filter">
				<div class="gmart-products-filter-form__filter-top">
					<div class="gmart-products-filter-form__filter-actions">
						<button type="button"
								class="gmart-products-filter-form__remove-filter button-link button-link-delete">
							<span class="screen-reader-text"><?php esc_html_e( 'Remove filter', 'gmart' ) ?></span>
							<span class="dashicons dashicons-no-alt"></span>
						</button>
					</div>

					<button type="button" class="gmart-products-filter-form__filter-toggle">
						<span class="gmart-products-filter-form__filter-toggle-indicator" aria-hidden="true"></span>
					</button>

					<div class="gmart-products-filter-form__filter-title"><?php echo $title; ?></div>
				</div>
				<div class="gmart-products-filter-form__filter-options">
					<?php
					foreach ( $filter_settings as $name => $options ) {
						...
						$options['class']      = 'gmart-products-filter-form__filter-option';
	...
	protected function get_filter_source_options() {
		$sources = array(
			'orderby'        => esc_html__( 'Order By', 'gmart' ),
			'products_group' => esc_html__( 'Group', 'gmart' ),
			'price'          => esc_html__( 'Price', 'gmart' ),
			'attribute'      => esc_html__( 'Attributes', 'gmart' ),
			'rating'         => esc_html__( 'Rating', 'gmart' ),
			'product_status' => esc_html__( 'Product Status', 'gmart' ),
		);
	...
	protected function get_filter_display_options( $source = 'product_cat' ) {
		$options = array(
			'price'     => array(
				'slider' => esc_html__( 'Slider', 'gmart' ),
				'ranges' => esc_html__( 'Ranges', 'gmart' ),
			),
			'attribute' => array(
				'auto'       => esc_html__( 'Auto', 'gmart' ),
				'dropdown'   => esc_html__( 'Dropdown', 'gmart' ),
				'list'       => esc_html__( 'Vertical List', 'gmart' ),
				'h-list'     => esc_html__( 'Horizontal List', 'gmart' ),
				'checkboxes' => esc_html__( 'Checkbox List', 'gmart' ),
			),
			'rating'    => array(
				'dropdown'   => esc_html__( 'Dropdown', 'gmart' ),
				'checkboxes' => esc_html__( 'Checkbox List', 'gmart' ),
	...
	protected function get_filter_fields_settings() {
		...
		$this->filter_settings = array(
			'name'        => array(
				'type'  => 'text',
				'label' => __( 'Filter Name', 'gmart' ),
			),
			'source'      => array(
				'type'    => 'select',
				'label'   => __( 'Filter By', 'gmart' ),
				'options' => $this->get_filter_source_options(),
			),
			'attribute'   => array(
				'type'      => 'select',
				'name'      => 'attribute',
				'label'     => __( 'Attribute', 'gmart' ),
			...
			'display'     => array(
				'type'    => 'select',
				'label'   => __( 'Display Type', 'gmart' ),
				'options' => $this->get_filter_display_options(),
			),
			'ranges'      => array(
				'type'      => 'textarea',
				'label'     => __( 'Ranges', 'gmart' ),
				'desc'      => __( 'Each range on a line, separate by the <code>-</code> symbol. Do not include the currency symbol.', 'gmart' ),
				'condition' => array(
					'display' => 'ranges',
					'source'  => 'price',
				),
			...
			'multiple'    => array(
				'type'      => 'select',
				'label'     => __( 'Selection Type', 'gmart' ),
				'options'   => array(
					0 => __( 'Single select', 'gmart' ),
					1 => __( 'Multiple select', 'gmart' ),
				),
				'condition' => array(
			...
			'show_counts' => array(
				'type'      => 'checkbox',
				'label'     => __( 'Show product counts', 'gmart' ),
				'condition' => array(
			...
			'searchable'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Show the search box', 'gmart' ),
				'condition' => array(
			...
			'scrollable'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Limit the height of items list (scrollable)', 'gmart' ),
				'condition' => array(
					'source!'  => array( 'product_status' ),
			...
			'show_empty_cats'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Show empty categories', 'gmart' ),
				'condition' => array(
					'source'  => array( 'product_cat' ),
				),
			),
			'disable_onsale'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Disable on sale', 'gmart' ),
				'condition' => array(
					'source'  => array( 'product_status' ),
				),
			),
			'disable_in_stock'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Disable in stock', 'gmart' ),
				'condition' => array(
					'source'  => array( 'product_status' ),
				),
			),
	...
	public function admin_scripts( $hook ) {
		...
		wp_enqueue_style( 'gmart-products-filter-admin', GMART_ASSETS_URL . '/assets/admin/products-filter-admin.css', array(), '20210311' );
//		wp_enqueue_script( 'gmart-products-filter-admin', GMART_ASSETS_URL . '/assets/admin/js/products-filter-admin.js', array( 'wp-util' ), '20210311', true );

		wp_localize_script(
			'gmart-products-filter-admin', 'dmt_products_filter_params', array(
				'sources'    => $this->get_filter_source_options(),
	...
	public function scripts() {
		...
		wp_enqueue_script( 'gmart-products-filter', GMART_ASSETS_URL . '/assets/admin/products-filter.js', array(
			'jquery',
			'wp-util',
			'select2',
	...
	public function filter_setting_fields_template() {
		...
		<script type="text/template" id="tmpl-gmart-products-filter">
