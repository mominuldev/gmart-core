...
class LogoList extends Widget_Base {
	...
	public function get_title() {
		return __( 'Logo List', 'gmart-core' );
	}
	...
	protected function register_controls() {
		...
		$repeater->add_control( 'logo_image', [
			'label'   => __( 'Choose Logo Image', 'gmart-core' ),
		...
		$repeater->add_control( 'brand_name', [
			'label'       => __( 'Brand Name', 'gmart-core' ),
		...
		$this->add_control( 'logo_lists', [
			...
			'default'     => [
				[
					'brand_name' => __( 'Brand Name', 'gmart-core' ),
					'logo_image' => [
		...
		$this->add_group_control( Group_Control_Box_Shadow::get_type(), [
			...
			'selector' => '{{WRAPPER}} .dmt-logo-list__item',
		] );
		...
		$this->add_responsive_control( 'brand_container_padding', [
			...
			'size_units' => [ 'px', '%', 'em' ],
			'selectors'  => [
				'{{WRAPPER}} .dmt-logo-list__item' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		] );
		...
		$this->add_responsive_control( 'border-radius', [
			...
			'size_units' => [ 'px', '%', 'em' ],
			'selectors'  => [
				'{{WRAPPER}} .dmt-logo-list__item' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		] );

		// Gap Between Brand with display grid
		$this->add_responsive_control( 'brand_gap', [
			'label'      => __( 'Gap Between Brand', 'gmart-core' ),
			...
			'selectors'  => [
				'{{WRAPPER}} .dmt-logo-list' => 'column-gap: {{SIZE}}{{UNIT}}; gap: {{SIZE}}{{UNIT}};',
			],
		] );

		// Min Height
		$this->add_responsive_control( 'brand_min_height', [
			'label'      => __( 'Min Height', 'gmart-core' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [ 'px', '%', 'em' ],
			'range'      => [
				'px' => [
			...
			'selectors'  => [
				'{{WRAPPER}} .dmt-logo-list__item' => 'min-height: {{SIZE}}{{UNIT}};',
			],
		] );
		...
		$this->add_control( 'brand_container_bg_color', [
			...
			'selectors' => [
				'{{WRAPPER}} .dmt-logo-list__item' => 'background-color: {{VALUE}};',
			],
		] );
		...
		$this->add_control( 'brand_container_hover_bg_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}} .dmt-logo-list__item:hover' => 'background-color: {{VALUE}};',
			],
		] );
		...
		$this->add_group_control( Group_Control_Border::get_type(), [
			...
			'label'    => __( 'Border', 'gmart-core' ),
			'selector' => '{{WRAPPER}} .dmt-logo-list__item:hover',
		] );
	...
	protected function render() {
		...
		if ( $settings['logo_lists'] ) { ?>
			<div class="client-logo-wrapper">
				<ul class="dmt-logo-list">
					<?php foreach ( $settings['logo_lists'] as $item ) { ?>
						<li class="dmt-logo-list__item wow fadeIn" data-bs-original-title="<?php echo esc_attr( $item['brand_name'] ) ?>">
							<a href="#">
