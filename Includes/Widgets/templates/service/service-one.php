<div <?php echo $this->get_render_attribute_string( 'wrapper' ) ?>>
	...
	<div class="dmt-service__content">
		<?php if ( $settings['title'] ): ?>
			<h4 class="dmt-service__title">
				<?php printf( '%s', $settings['title'] ); ?>
			</h4>
		<?php endif; ?>

		<?php if ( !empty($settings['button_text']) && 'one' == $settings['layout'] ): ?>
			<a href="<?php echo esc_url( $settings['button_link']['url'] ); ?>" class="dmt-service__btn">
				<?php printf( '%s', $settings['button_text'] ); ?>
			</a>
		<?php endif; ?>

		<?php if ( !empty( $settings['button_link']['url'] ) && 'two' == $settings['layout'] ): ?>
			<a href="<?php echo esc_url( $settings['button_link']['url'] ); ?>" class="dmt-service__arrow">
