...
if ( ! class_exists( 'Dmt_Helper' ) ) {
	class Dmt_Helper {
		...
		public function initialize() {
			add_filter('upload_mimes', [ $this, 'dmt_svg_mime_types' ]);
			add_action( 'show_user_profile', [$this, 'dmt_extra_user_profile_fields'] );
			add_action( 'edit_user_profile', [$this, 'dmt_extra_user_profile_fields'] );
			add_action( 'personal_options_update', [$this, 'dmt_save_extra_user_profile_fields'] );
			add_action( 'edit_user_profile_update', [$this, 'dmt_save_extra_user_profile_fields'] );
		}
		 ...
		 * Get Contact form 7
		 */
		public static function dmt_get_contact_form7_forms(){
			$forms = get_posts( 'post_type=wpcf7_contact_form&posts_per_page=-1' );

			$results = array();
			if ( $forms ) {
				$results[] = __( 'Select A Form', 'gmart-core' );
				foreach ( $forms as $form ) {
					$results[ $form->ID ] = $form->post_title;
				}
			} else {
				$results[] =  __( 'No contact forms found', 'gmart-core' ) ;
			}
		 ...
		 * @return void
		 */
		public function dmt_extra_user_profile_fields( $user ) { ?>
			<h3><?php esc_html_e( "Extra profile information", 'gmart-core' ); ?></h3>

			<table class="form-table">
				<tr>
					<th><label for="facebook"><?php esc_html_e( "Facebook", 'gmart-core' ); ?></label></th>
					<td>
						<input type="text" name="facebook" id="facebook" value="<?php echo esc_attr( get_the_author_meta( 'facebook', $user->ID ) ); ?>" class="regular-text"/><br/>
						<span class="description"><?php esc_html_e( "Please enter your facebook url.", 'gmart-core' ); ?></span>
					</td>
				</tr>
				<tr>
					<th><label for="twitter"><?php esc_html_e( "Twitter", 'gmart-core' ); ?></label></th>
					<td>
						<input type="text" name="twitter" id="twitter" value="<?php echo esc_attr( get_the_author_meta( 'twitter', $user->ID ) ); ?>" class="regular-text"/><br/>
						<span class="description"><?php esc_html_e( "Please enter your twitter url.", 'gmart-core' ); ?></span>
					</td>
				</tr>
				<tr>
					<th><label for="linkedin"><?php esc_html_e( "Linkedin", 'gmart-core' ); ?></label></th>
					<td>
						<input type="text" name="linkedin" id="linkedin"  value="<?php echo esc_attr( get_the_author_meta( 'linkedin', $user->ID ) ); ?>"  class="regular-text"/><br/>
						<span class="description"><?php esc_html_e( "Please enter your linkedin url.", 'gmart-core' ); ?></span>
					</td>
				</tr>

				<tr>
					<th><label for="instagram"><?php esc_html_e( "Instagram", 'gmart-core' ); ?></label></th>
					<td>
						<input type="text" name="instagram" id="instagram" value="<?php echo esc_attr( get_the_author_meta( 'instagram', $user->ID ) ); ?>"  class="regular-text"/><br/>
						<span class="description"><?php esc_html_e( "Please enter your instagram url.", 'gmart-core' ); ?></span>
					</td>
				</tr>
			</table>
		<?php }
		 ...
		 * @return false|void
		 */
		public function dmt_save_extra_user_profile_fields( $user_id ) {
		...
		static public function dmt_category_list( $cat ) {
			$query_args = array(
				'orderby'    => 'ID',
				'order'      => 'DESC',
				'hide_empty' => 1,
...
//			$options     = array( esc_html__( '0', 'gmart-core' ) => 'All Category' );
			$options = [];
		...
		public static function categories_suggester() {
			...
			foreach (get_categories() as $cat) {
				...
				$content[(string) $cat->slug] = $cat->cat_name.(! empty($parent) ? esc_html__( ' (Parent categories: (', 'gmart-core') .$parent.'))' : "");
			}
		...
		public static function get_grid_metro_size() {
			return [
				'1:1'   => esc_html__( 'Width 1 - Height 1', 'gmart-core' ),
				'1:2'   => esc_html__( 'Width 1 - Height 2', 'gmart-core' ),
				'1:0.7' => esc_html__( 'Width 1 - Height 70%', 'gmart-core' ),
				'1:1.3' => esc_html__( 'Width 1 - Height 130%', 'gmart-core' ),
				'2:1'   => esc_html__( 'Width 2 - Height 1', 'gmart-core' ),
				'2:2'   => esc_html__( 'Width 2 - Height 2', 'gmart-core' ),
			];
		}
		 ...
		 * @return void
		 */
		function dmt_svg_mime_types($mimes) {
		...
		public static function image_placeholder( $width, $height ) {
			echo '<img src="http://via.placeholder.com/' . $width . 'x' . $height . '?text=' . esc_attr__( 'No+Image', 'gmart-core' ) . '" alt="' . esc_attr__( 'Thumbnail', 'gmart-core' ) . '"/>';
		}

		public static function dmt_hex_to_rgb($hex, $alpha = false) {
			$hex      = str_replace('#', '', $hex);
			$length   = strlen($hex);
	...
	Dmt_Helper::instance()->initialize();
}