...
class WidgetProductFilter extends \WP_Widget {

	function __construct() {
		/* Widget settings. */
		$widget_ops = array(
			'classname'   => 'dmt_ajax_filte',
			'description' => __( 'Allows you to filter atribute,price products', 'gmart-core' )
		);

		/* Widget control settings. */
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'dmt_ajax_filte' );

		/* Create the widget. */
		parent::__construct( 'dmt_ajax_filte', __( 'DMT Product Ajax Filter', 'gmart-core' ), $widget_ops, $control_ops );
		add_action( 'admin_enqueue_scripts', array( $this, 'dmt_filter_admin_script' ) );
		add_action( 'dmt_woocommerce_product_filter_result', array( $this, 'dmt_filter_title' ) );
		/* Ajax Call*/
		add_action( 'wp_ajax_dmt_filter_products_callback', array( $this, 'dmt_filter_products_callback' ) );
		add_action( 'wp_ajax_nopriv_dmt_filter_products_callback', array( $this, 'dmt_filter_products_callback' ) );
	}
	...
	function dmt_filter_title() {
		...
		<div class="woocommerce-filter-title">
			...
			<?php if ( $check ): ?>
				<button class="filter_clear_all dmt-clear-btn" type="button"><?php echo esc_html__( 'Clear All', 'gmart-core' ); ?></button>
			<?php endif;
			?>
		</div>
		<?php
	...
	function dmt_filter_products_callback() {
		global $wpdb;
		$dir = DMT_WIDGET_TEMPLATE_PATH . 'dmt-ajax-filter/default_ajax.php';
		include $dir;
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( ! in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
			_e( 'Please active woocommerce plugin or install woomcommerce plugin first!', 'gmart-core' );
	...
	public function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title1' ); ?>"><?php _e( 'Title', 'gmart-core' ) ?></label>
			<br/>
		...
		<p>
			<label
				for="<?php echo $this->get_field_id( 'show_category' ); ?>"><?php _e( "Show Categories", 'gmart-core' ) ?></label>
			<br/>
			<select class="widefat"
				...
				<?php } ?>>
					<?php _e( 'Yes', 'gmart-core' ) ?>
				</option>
				<option value="0" <?php if ( $show_category == 0 ) { ?> selected="selected"
				<?php } ?>>
					<?php _e( 'No', 'gmart-core' ) ?>
				</option>
			</select>
		</p>
		<p>
			<label
				for="<?php echo $this->get_field_id( 'show_price' ); ?>"><?php _e( "Show Filter Price", 'gmart-core' ) ?></label>
			<br/>
			<select class="widefat"
		...
		<p>
			<label
				for="<?php echo $this->get_field_id( 'showcount' ); ?>"><?php _e( "Show Number Of Product", 'gmart-core' ) ?></label>
			<br/>
			<select class="widefat" id="<?php echo $this->get_field_id( 'showcount' ); ?>"
		...
		<p>
			<label
				for="<?php echo $this->get_field_id( 'show_brand' ); ?>"><?php _e( "Show Brand", 'gmart-core' ) ?></label>
			<br/>
			<select class="widefat"
					id="<?php echo $this->get_field_id( 'show_brand' ); ?>"
					name="<?php echo $this->get_field_name( 'show_brand' ); ?>">
		...
		<?php if ( $attribute_taxonomies && $count_atribute > 0 ) : ?>
			<p>
				<label
					for="<?php echo $this->get_field_id( 'attribute' ); ?>"><?php _e( 'Attribute', 'gmart-core' ) ?></label>
				<br/>
				<select class="widefat" id="<?php echo $this->get_field_id( 'attribute' ); ?>"
						name="<?php echo $this->get_field_name( 'attribute' ); ?>[]" multiple="multiple">
					<option value=""><?php echo esc_html__( 'Null', 'gmart-core' ) ?></option>
					<?php
					$option = '';
					foreach ( $attribute_taxonomies as $value ) :
		...
		<table>
			...
			foreach ( $attribute as $att ) {
				if ( preg_match( '/' . __( 'color', 'gmart-core' ) . '/', $att, $matches ) ) { ?>
					<?php
				...
				if ( $count_terms > 0 ) {
					if ( count( $terms ) > 0 ):
						...
						<div class="dmt-filter dmt-filter-<?php echo esc_attr( $att ); ?>">
							<h3>
								<?php echo ucfirst( $name_att ); ?><?php if ( count( $current_values ) > 0 ): ?>
									<label class="count-chosen"><?php echo count( $current_values ); ?></label>
								<?php endif; ?>
							</h3>
							<div class="content_filter">
								...
								<?php } else { ?>
									<!--									<h2 class="dropdown-toggle" data-toggle="dropdown">--><?php //echo esc_html__('Choose option','gmart-core')
									?><!--</h2>-->
									<ul id="<?php echo esc_attr( 'pa_' . $att ); ?>"
	...
	function woocommerce_filter_category() {
		...
		if ( $terms ) {
			...
			foreach ( $terms as $term ) {
				...
				echo '<a href="' . get_term_link( $term->term_id, 'product_cat' ) . '"><span class="name">' . esc_html( $term->name ) . '</span>';
				echo '<span class="count">(' . ( $term->count ) . ')</span></a>';
				if ( $terms_vl1 ) {
					...
					foreach ( $terms_vl1 as $term_vl ) {
						...
						echo '<a href="' . get_term_link( $term_vl->term_id, 'product_cat' ) . '"><span class="name">' . esc_html( $term_vl->name ) . '</span>';
						echo '<span class="count">(' . ( $term_vl->count ) . ')</span></a>';
						if ( $terms_vl2 ) {
							echo '<div class="children">';
							foreach ( $terms_vl2 as $term_vl2 ) {
								...
								echo '<a href="' . get_term_link( $term_vl2->term_id, 'product_cat' ) . '"><span class="name">' . esc_html( $term_vl2->name ) . '</span>';
								echo '<span class="count">(' . ( $term_vl2->count ) . ')</span></a>';
								echo '</div> ';
							}
							echo '</div>';
						}
	...
	function woocommerce_filter_brand( $showcount ) {
		...
		if ( $terms ) {
			...
			foreach ( $terms as $term ) {
				...
				echo '<a href="' . get_term_link( $term->term_id, 'product_brand' ) . '"><span class="name">' . esc_html( $term->name ) . '</span>';
				echo '<span class="count">(' . ( $term->count ) . ')</span></a>';
				echo '</div> ';
			}
			echo '</div></div>';
