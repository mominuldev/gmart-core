...
<nav class="woocommerce-pagination <?php echo esc_attr($shop_paging); ?>">
	<div class="woocommerce-product-count">
		...
		if ( 1 === $total ) {
			_e( 'Showing the single result', 'gmart-core' );
		} elseif ( $total <= $per_page || -1 === $per_page ) {
			printf( __( 'Showing all %d results', 'gmart-core' ), $total );
		} else {
			printf( _x( 'Showing %1$d&ndash;%2$d of %3$d item(s)', '%1$d = first, %2$d = last, %3$d = total', 'gmart-core' ), $first, $last, $total );
		}
		?>
	</div>
	...
	<div class="loadmore">
		<button type="button" class="woocommerce-load-more" data-paged="<?php echo esc_attr($paged); ?>">
			...
			<span class="loadmore-button-text"><?php echo esc_html__('Load More', 'gmart-core'); ?></span>
		</button>
	</div>
</nav>
