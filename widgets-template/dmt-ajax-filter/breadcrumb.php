...
if ( $tax_name == "product_cat" ) {
	...
	if ( $list ) {
		echo '<a href="' . esc_url( $home ) . '">' . esc_html__( 'Home', 'gmart-core' ) . '</a>' . wp_kses( $delimiter, 'social' ) . esc_html( $taxonomy_details->labels->singular_name ) . '</a>' . wp_kses( $delimiter, 'social' ) . $list;
	}
	echo '</div>';
	echo '</div>';
}
